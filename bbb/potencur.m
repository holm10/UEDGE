...
************************************************************************
     ...
     .                   torcurdir*(rr(ix1,iy)*t0**1.5*gx(ix1,iy)
     ...
     .                   / ((gx(ix1,iy)+gx(ix,iy))*ev**1.5)
               ...
               sigbarx = zfac * cfsigm * sigma1 * torcurdir*rrv(ix,iy) *
     .                                 ( 0.5*(t0+t1)/ev )**1.5
            endif
...
c    .              4.0*(prtv(ix,iy) - prtv(ix,iy1)) * gyc(ix,iy)
            fqp(ix,iy) = (torcurdir*rrv(ix,iy)*sx(ix,iy)*sigbarx*gxf(ix,iy)/qe)*
     .                       ( (pre(ix1,iy) - pre(ix,iy))/nbarx
     .                  - qe * (phi(ix1,iy) - phi(ix,iy))
     .                  + qe * (pondpot(ix1,iy) - pondpot(ix,iy))
     .                  + pondomfpare_use(ix,iy)/(torcurdir*rrv(ix,iy)*nbarx)
...
c           old case with frfqpn=0 ...
            do jx = 1, nxpt  # loop over all mesh regions
               ...
               if (ix==ixl .and. ixmnbcl==1) then  # at left boundary
                  fqp_old = fqp(ix,iy)
                  nbarx = ne(ixlp1,iy)
                  sigbarx = zfac*cfsigm*sigma1*torcurdir*rrv(ixlp1,iy)*
     .                                         (te(ixlp1,iy)/ev)**1.5
                  fqp(ix,iy) =
     .                  (torcurdir*rrv(ixlp1,iy)*sx(ixlp1,iy)*sigbarx*gxf(ixlp1,iy)/qe)*
     ...
     .                         up(ixl,iy,ifld)*sx(ixl,iy)*rrv(ixl,iy)*torcurdir
                  enddo
     ...
     .                              abs(fp2)**exjbdry) )**(1/exjbdry)
               ...
               elseif (ix==ixrm1 .and. ixmxbcl==1) then  # at right boundary
                  fqp_old = fqp(ix,iy)
                  nbarx = ne(ixrm1,iy)
                  sigbarx = zfac*cfsigm*sigma1*torcurdir*rrv(ixrm2,iy)*
     .                                         (te(ixrm1,iy)/ev)**1.5
                  fqp(ix,iy) =
     .                  (torcurdir*rrv(ixrm2,iy)*sx(ixrm2,iy)*sigbarx*gxf(ixrm2,iy)/qe)*
     ...
     .                        up(ixrm1,iy,ifld)*sx(ixrm1,iy)*rrv(ixrm1,iy)*torcurdir
                  enddo
     ...
     .                                abs(fp2)**exjbdry) )**(1/exjbdry)
            ...
            if (iy .eq. 1) then
               dphi_iy1(ix) = -fqp(ix,iy)/(torcurdir*rrv(ix,iy)*sx(ix,iy)*
     .                         sigbarx*gxf(ix,iy)) +
