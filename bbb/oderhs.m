...
c ... Calc friction forces from Braginskii; no individ chg-states;isimpon < 5.
     ...
     .                   lcone(ix,iy) )
               ...
               frice(ix,iy) = -cthe*flxlimf*nbarx*torcurdir*rrv(ix,iy)*gtex(ix,iy) +
...
c     cfyef is included. Both isphiofft=0 or 1 cases included in one loop
     ...
     .                                              gpondpotx(ix,iy) +
     .                 pondomfpare_use(ix,iy)/(qe*torcurdir*rrv(ix,iy)*nexface) ) + 
     .                      isphiofft * (
...
c     saved here so they can be restored below.
     ...
     .                       (upi(ix1,iy,ifld)-upi(ix1,iy,1))*exp(-argx)
               ...
               uup(ix1,iy,ifld) = torcurdir*rrv(ix1,iy)*upi(ix1,iy,ifld)
            enddo
         endif
     ...
     .                         (upi(ix,iy,ifld)-upi(ix,iy,1))*exp(-argx)
               ...
               uup(ix,iy,ifld) = torcurdir*rrv(ix,iy)*upi(ix,iy,ifld)
            enddo
         enddo
      enddo
...
c     to those from parallel flow.
     ...
     .                       /(ni(ix2,iy,ifld)+ni(ix,iy,ifld))
               uz(ix,iy,ifld) = -uup(ix,iy,ifld)/(torcurdir*rrv(ix,iy))*
...
c ..          first left plate(s)
                if(isfixlb(jx) == 0) then # set upi for left plate
                  cs = torcurdir*(csfacrb(ifld,jx)*sqrt( (te(ixt0,iy) +
     .                            csfacti*ti(ixt0,iy))/mi(ifld) ))
                  ueb = cfueb*( cf2ef*v2ce(ixt0,iy,ifld)*rbfbt(ixt0,iy) -
     .                            vytan(ixt0,iy,ifld) )/(torcurdir*rrv(ixt0,iy))
	          uu(ixt0,iy,ifld) = -rrv(ixt0,iy)*cs*torcurdir
                  upi(ixt0,iy,ifld) = -(cs - ueb)
                endif
c ..          switch to right plate(s)
                if(isfixrb(jx) == 0) then
                  cs = torcurdir*(csfacrb(ifld,jx)*sqrt( (te(ixt1,iy) +
     .                            csfacti*ti(ixt1,iy))/mi(ifld) ))
                  ueb = cfueb*( cf2ef*v2ce(ixt1,iy,ifld)*rbfbt(ixt,iy) -
     .                            vytan(ixt1,iy,ifld) )/(torcurdir*rrv(ixt1,iy))
	          uu(ixt1,iy,ifld) = rrv(ixt1,iy)*cs*torcurdir
...
************************************************************************
   ...
   27 continue
     ...
     .                               (torcurdir*rrv(ix,iy)*sx(ix,iy)*qe))/
  ...
  29  continue
	 ...
	 do 730 ix = i1, i6
            ix1 = ixp1(ix,iy)
            vex(ix,iy) = upe(ix,iy)*torcurdir*rrv(ix,iy) + 
...
*  -- Set up electron parallel contribution to seec & smoc
      do iy = j2, j5
         do ix = i2, i5
            ...
            if (oldseec .gt. 0) then
                t1 =.5*cvgp*(upe(ix,iy)*torcurdir*rrv(ix,iy)*
     .                  ave(gx(ix,iy),gx(ix2,iy))*gpex(ix,iy)/gxf(ix,iy) +
     .                  upe(ix1,iy)*torcurdir*rrv(ix1,iy)*
     ...
     .                   qe*nexface*gpondpotx(ix,iy) )*torcurdir*rrv(ix,iy)  +
...
*     -- (note sign change in pondomfpari_use term starting 031722)
           do 31 iy = j2, j5
             do 30 ix = i2, i5
               ...
               t1 = .5*cvgp*( up(ix,iy,ifld)*torcurdir*rrv(ix,iy)*
     .                                 ave(gx(ix2,iy),gx(ix,iy))*tv
     .                      + up(ix1,iy,ifld)*torcurdir*rrv(ix1,iy)*
     .                                 ave(gx(ix,iy),gx(ix1,iy))*t1 )
               seic(ix,iy) = seic(ix,iy) + cfvgpx(ifld)*t1*vol(ix,iy)
               t0 = - cpiup(ifld)*( gpix(ix,iy,ifld)*torcurdir*rrv(ix,iy) -
     ...
     .                       ni(ix,iy,ifld) )*ex(ix,iy)*torcurdir*rrv(ix,iy) +
     ...
     .                        vyg(ix,iy1,1)*gpiy(ix,iy1,2) )*vol(ix,iy)
          ...
          do iy = j2, j5
            do ix = i2, i5
               ...
               t1 = .5*cvgp*( up(ix,iy,2)*torcurdir*rrv(ix,iy)*
     .                               ave(gx(ix2,iy),gx(ix,iy))*tv
     .                    + up(ix1,iy,2)*torcurdir*rrv(ix1,iy)*
     .                               ave(gx(ix,iy),gx(ix1,iy))*t1 )
...
c...  visx(nx+1,iy) as they are meaningless when flux limited
               ...
               t0 = max (ti(ix,iy), temin*ev)
               vtn = sqrt(t0/mi(ifld))
               mfl = flalfv * nm(ix,iy,ifld) * torcurdir*rr(ix,iy) *
...
c ... Flux limit individ. hcxij in poloidal direction if isflxldi=2
     ...
     .                    .and. (isplflxl==0) ) wallfac = flalfipl/flalfi
                  enddo
                  qflx = wallfac*flalfi * torcurdir*rrv(ix,iy) *
     ...
     .                                (ti(ix,iy)-ti(ix1,iy))/(rrv(ix,iy)*torcurdir)
            enddo
         enddo
 595  continue 
...
*  -- compute fnix --
     ...
     .                                   1.-torcurdir*rrv(ix,iy) > 1.e-4 ) then
         ...
         do 91 iy = j4, j8
            ...
            do 90 ix = i2, i6
               ...
               if (isimpon.ge.5 .and. ifld.eq.1) then
                  ...
                  uuv = 0.5*( torcurdir*(up(ix1,iy,ifld)*rrv(ix1,iy)+
...
c ...   First, the short mfp drag 
     ...
     .                                      gxf(ix1,iy)*torcurdir*rrv(ix1,iy)
            dbds_p = (btot(ix2,iy) - btot(ix,iy))*
     .                                        gxf(ix,iy)*torcurdir*rrv(ix,iy)
...
*  -- source term and pressure gradient --
     ...
     .                - cfneut * cfneutsor_mi * sx(ix,iy) * torcurdir * rrv(ix,iy) * dp1
...
c.... Now do the ions (hcxi is flux-limited previously when it is built)
     ...
     .             ( abs(ti(ix,iy)-ti(ix2,iy))*gxf(ix,iy) + 100.*cutlo)
	    qsh = csh * (ti(ix,iy)-ti(ix2,iy)) * (1. + lxtic/lxtimax)
            qipar(ix,iy,1) = qsh/(torcurdir*rrv(ix,iy)*sx(ix,iy))
...
c     there are array locations.
      amu(1) = 5.45e-4
      gradp(1,1) = flxlimf*torcurdir*rrv(ix,iy) * gpex(ix,iy)
      gradt(1,1) = flxlimf*torcurdir*rrv(ix,iy) * gtex(ix,iy)
...
c        of pressure and Ti at poloidal density-cell faces.
         do nz = 1, natomic(misa)
            ...
            gradp(misa,nz) = flxlimf*torcurdir*rrv(ix,iy) * gpix(ix,iy,ifld)
            tempa(misa) = 0.5 * (ti(ix,iy) + ti(ix1,iy))
            gradt(misa,nz) = flxlimf*torcurdir*rrv(ix,iy) * gtix(ix,iy) 
...
c ... Set up other inputs for fmombal, including flow velocities.
 50   epar = flxlimf*torcurdir*rrv(ix,iy) * ex(ix,iy)
      umassni = 0.
      massni = 0.
      do ifld = 1, nusp
...
c     nothing with qcond.
      fqp(ix,iy) = cfparcur*parcurrent * torcurdir*rrv(ix,iy)*sx(ix,iy)
...
c     of gas, and a few other (physically meaningless) quantities for which
c     there are array locations.
      gradp(1,1) = torcurdir*rrv(ix,iy) * gpex(ix,iy)
      gradt(1,1) = torcurdir*rrv(ix,iy) * gtex(ix,iy)
...
ccc   For arbitrary impurity concentration use the following frici:
     ...
     .                         torcurdir*rrv(ix,iy)*0.5*(ne(ix,iy)+ne(ix1,iy)) )
...
c ... of pressure and Ti at poloidal density-cell faces.
         do nz = 1, natomic(misa)  # note: hydrogen friction set above
            ...
            den(misa,nz) = 0.5 * (ni(ix,iy,ifld) + ni(ix1,iy,ifld))
            gradp(misa,nz) = torcurdir*rrv(ix,iy) * gpix(ix,iy,ifld) -
     .                                      pondomfpari_use(ix,iy,ifld)
            tempa(misa) = 0.5 * (ti(ix,iy) + ti(ix1,iy))
            gradt(misa,nz) = torcurdir*rrv(ix,iy) * gtix(ix,iy) 
...
c... NOTE:next coefficient 12*pi*sqrt(pi/2)*epsilon**2/e**4 = 5.624e54 in mks
     ...
     .                         + qe*zi(ifld)*torcurdir*rrv(ix,iy)*ex(ix,iy) 
     .                         + volmsor(ix,iy,ifld)/
...
c ...       No flxlimf for 1st option; it only enhances (1/taudeff)
     ...
     .                           torcurdir*rrv(ix,iy)*ex(ix,iy) + gradp(misa,nz)  
...
c --- which has no non-orthogonal part.
            ...
            if (isupgon(igsp) .eq. 1) then
              qtgf = qtgf + torcurdir*rrv(ix,iy)*up(ix,iy,iigsp)*sx(ix,iy)
            endif
...
c ... methn
      do iy = j1, j5
         do ix = i1,i5
            ix1 = ixp1(ix,iy)
            if (1.-torcurdir*rrv(ix,iy) > 1.e-4) then #combine binormal & par components
...
c ... methn
      do iy = j1, j5
        do ix = i1,i5
          ix1 = ixp1(ix,iy)
          if (1.-torcurdir*rrv(ix,iy) > 1.e-4 .or. isupgon(igsp)==0) then 
...
c --- use (Bt/B)**2=1-rrv**2 instead.
         do 23 iy = j4, j6
            do 22 ix = i1, i6
               ix2 = ixp1(ix,iy)
               uu(ix,iy,iigsp) = torcurdir*rrv(ix,iy)*up(ix,iy,iigsp) +
...
cc            ptjdote = ptjdote + jdote(ix,iy)
            ...
            if (isupgon(1) .eq. 0) then
               pmomv(ix,iy)=cngmom(1)*up(ix,iy,1)*sx(ix,iy)*torcurdir*rrv(ix,iy)*
     .                           ( ng(ix2,iy,1)*tg(ix2,iy,1)- 
     .                             ng(ix ,iy,1)*tg(ix ,iy,1) ) +
...
# includes data for charged species; inertial neutrals are excluded.
     ...
     .      call gfsub3 (nunit,nx,ny,nxd,nyd,1,upi(0:nxd+1,0:nyd+1,ifld))
      enddo
      call gfsub3 (nunit,nx,ny,nxd,nyd,1,torcurdir*rr(0:nxd+1,0:nyd+1))
      do ifld = 1, nisp
         if (zi(ifld) .gt. 0) 
      ...
      do ifld=1,nisp
         do ix=1,nx
            do iy=1,ny
               ...
               uuc(ix,iy,ifld)=upc(ix,iy,ifld)*torcurdir*rr(ix,iy)
     .                         +v2c(ix,iy,ifld)*rbfbt(ix,iy)
               utc(ix,iy,ifld)=upc(ix,iy,ifld)*rbfbt(ix,iy)
     .                         -v2c(ix,iy,ifld)*torcurdir*rr(ix,iy)
            enddo
         enddo
      enddo
...
c compute UEDGE flow velocity at left target plate:
      do ifld=1,nisp
         do iy=1,ny
            ...
            # from these we construct the poloidal and toroidal components:
            uutg1(iy,ifld)=uptg1(iy,ifld)*torcurdir*rr(ixt,iy)
     .                      +v2tg1(iy,ifld)*rbfbt(ixt,iy)
            uttg1(iy,ifld)=uptg1(iy,ifld)*rbfbt(ixt,iy)
     .                      -v2tg1(iy,ifld)*torcurdir*rr(ixt,iy)
         enddo
      enddo
      if (mhdgeo .eq. 1) then		# convert to cylindrical coordinates:
         do ifld=1,nisp
...
c compute UEDGE flow velocity at right target plate:
      do ifld=1,nisp
         do iy=1,ny
            ...
            # from these we construct the poloidal and toroidal components:
            uutg2(iy,ifld)=uptg2(iy,ifld)*torcurdir*rr(ixt,iy)
     .                      +v2tg2(iy,ifld)*rbfbt(ixt,iy)
            uttg2(iy,ifld)=uptg2(iy,ifld)*rbfbt(ixt,iy)
     .                      -v2tg2(iy,ifld)*torcurdir*rr(ixt,iy)
         enddo
      enddo
      if (mhdgeo .eq. 1) then		# convert to cylindrical coordinates:
         do ifld=1,nisp
...
c     First do the viscosity driven by particle flux            
     ...
     .                                       rbfbt(ix1,iy)/(torcurdir*rrv(ix1,iy)) )
            tempm = b12(ix,iy)*( up(ix,iy,ifld) + 
     .                (v2cd(ix,iy,ifld)+v2ce(ix,iy,ifld))*
     .                                       rbfbt(ix,iy)/(torcurdir*rrv(ix,iy)) )
            if(ix < nx) then
              diffp = (visxneo(ix1,iy,ifld)/(torcurdir*rr(ix1,iy)))*(tempp-tempm)*
     ...
     .                                       rbfbt(ix2,iy)/(torcurdir*rrv(ix2,iy)) )
            diffm = (visxneo(ix,iy,ifld)/(torcurdir*rr(ix,iy)))*(tempp-tempm)*
     .                                             gx(ix,iy)/bsqr(ix,iy)
            visvol_v(ix,iy,ifld)=(4./3.)*torcurdir*rrv(ix,iy)*
     .                                 b32(ix,iy)*(diffp-diffm)*
     .                                   gxf(ix,iy)*volv(ix,iy)
c     Now do the viscosity driven by heat flux            
            tempp = b12(ix1,iy)*( qipar(ix1,iy,ifld) + 
     .                   q2cd(ix1,iy,ifld)*rbfbt(ix1,iy)/(torcurdir*rrv(ix1,iy)) )
            tempm = b12(ix,iy)*( qipar(ix,iy,ifld) + 
     .                   q2cd(ix,iy,ifld)*rbfbt(ix,iy)/(torcurdir*rrv(ix,iy)) )
            if(ix < nx) then
              diffp = alfneo(ix1,iy,ifld)*torcurdir*rr(ix1,iy)*(tempp-tempm)*
     ...
     .                   q2cd(ix2,iy,ifld)*rbfbt(ix2,iy)/(torcurdir*rrv(ix2,iy)) )
            diffm = alfneo(ix,iy,ifld)*torcurdir*rr(ix,iy)*(tempp-tempm)*
     .                   gx(ix,iy) / (nuii(ix,iy,ifld)*bsqr(ix,iy))
            visvol_q(ix,iy,ifld) = torcurdir*rrv(ix,iy)*b32(ix,iy)*
...
c     First do the current driven by neoclassical particle flux            
     ...
     .                        (v2cd(ix,iy,ifld)+v2ce(ix,iy,ifld))*
     .                                     rbfbt(ix,iy)/(torcurdir*rrv(ix,iy))
            gradx_vpiv = (visxneo(ix,iy,ifld)*b12(ix,iy)*torcurdir*rrv(ix,iy)/3.)
...
c     Now do the current driven by neoclassical heat flux            
     ...
     .                     b12(ix,iy)*torcurdir*rrv(ix,iy)/nuii(ix,iy,ifld) )*
     .                     ( (tempp+temp0)*b12ctr(ix1,iy)-
     .                       (temp0+tempm)*b12ctr(ix,iy) )*0.5*gxf(ix,iy)
